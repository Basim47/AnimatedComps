...
import Button from "@components/bouncyOnboarding/Button";
import { bouncyData } from "@components/bouncyOnboarding/data";
import { WIDTH } from "@utils/device";
import Paginator from "@components/bouncyOnboarding/Paginator";
import { ReText } from "react-native-redash";
import BouncyOnbarodingItem from "@components/bouncyOnboarding/BouncyOnboardingItem";
const AnimatedPressable = Animated.createAnimatedComponent(Pressable);

const BouncyOnboardingScreen = () => {
    ...
    const handleScroll = (direction: "left" | "right") => {
        ...
        if (newIndex >= 0 && newIndex < bouncyData.length) {
            ...
            const newScrollX = WIDTH * newIndex;
    ...
    const setButtonWidthAndLabel = () => {
        if (activeIndex === bouncyData.length - 1) {
            buttonWidth.value = withDelay(150, withTiming((WIDTH / 3) * 2));
    ...
    const handleSkipPress = () => {
        ...
        const newScrollX = WIDTH * (bouncyData.length - 1);
        scrollX.value = newScrollX;
        setTimeout(() => {
            scrollViewRef.current?.scrollTo({
                x: newScrollX,
...
export default BouncyOnboardingScreen;
